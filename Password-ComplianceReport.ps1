...
$AllUsers = Get-ADUser -Filter {Enabled -eq $true} -Properties `
    ...
    PasswordNotRequired, `     # Whether a password is even required for this account
    PasswordLastSet, `         # The date the password was last changed
    SamAccountName, `          # The username
    DisplayName                # The full display name
...
foreach ($User in $AllUsers) {  # Loops through every enabled account one by one
    ...
    if ($Issues.Count -gt 0) {
        $NonCompliant += [PSCustomObject]@{
            Username     = $User.SamAccountName  # The account username
